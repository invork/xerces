 ...
 * a character stream available, the parser will read that stream directly; if not, the parser will
 * use a byte stream, if available; if neither a character stream nor a byte stream is available,
 * the parser will attempt to open a URI connection to the resource identified by the system
 * identifier.
 *
 * <p>An InputSource object belongs to the application: the SAX parser shall never modify it in any
 * way (it may modify a copy if necessary).
 *
 * @since SAX 1.0
...
public class InputSource {
   ...
   *
   * <p>Application writers may use setSystemId to provide a base for resolving relative URIs,
   * setPublicId to include a public identifier, and/or setEncoding to specify the object's
   * character encoding.
   *
   ...
   *
   * <p>Application writers may use setSystemId() to provide a base for resolving relative URIs, and
   * setPublicId to include a public identifier.
   *
   ...
   *
   * <p>The public identifier is always optional: if the application writer includes one, it will be
   ...
   *
   * <p>The system identifier is optional if there is a byte stream or a character stream, but it is
   * still useful to provide one, since the application can use it to resolve relative URIs and can
   * include it in error messages and warnings (the parser will attempt to open a connection to the
   * URI only if there is no byte stream or character stream specified).
   *
   ...
   *
   * <p>If the system ID is a URL, it must be fully resolved.
   *
   ...
   *
   * <p>The getEncoding method will return the character encoding of the object pointed to, or null
   * if unknown.
   *
   ...
   *
   * <p>The encoding must be a string acceptable for an XML encoding declaration (see section 4.3.3
